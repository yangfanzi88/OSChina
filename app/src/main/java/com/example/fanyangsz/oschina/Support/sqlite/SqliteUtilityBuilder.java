...
public class SqliteUtilityBuilder {
    ...
    public SqliteUtility build(Context context) {
        ...
        if (sdcardDb) {
            ...
            Log.d(TAG, String.format(String.format("打开sdcard库 %s, version = %d", dbName, db.getVersion())));
        } else {
            db = new SqliteDbHelper(context, dbName, version).getWritableDatabase();

            Log.d(TAG, String.format(String.format("打开app库 %s, version = %d", dbName, db.getVersion())));
        }
